...
fn main() {
    ...
    let container: BTreeMap<i64,i64> = transform_data_from_csv(PATH_TO_CSV).unwrap();
    // println!("container: {:?}", container);

    write_data_to_csv(container, PATH_TO_CSV_DATA).expect(&format!("Unable to write data to csv file {PATH_TO_CSV_DATA}"));
